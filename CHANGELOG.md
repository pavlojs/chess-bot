...
- **Full-power mate execution:** Added `_get_full_power_move` helper. When a forced mate is detected (positive or negative), the engine temporarily disables `UCI_LimitStrength`, searches at full power to find the fastest/best mate or the strongest defensive resource, then restores the strength cap. Recovery search is unaffected and continues to run at the limited ELO — only the mate branches use the full-strength override.
