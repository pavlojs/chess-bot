...
- **Recovery with ELO boost:** Added `PREDICTION_RECOVER_ELO_BOOST` (default: 200). Recovery search keeps `UCI_LimitStrength` active but raises the ELO cap by this amount (e.g. 1600 → 1800 during recovery), giving the bot a meaningfully stronger defensive resource while keeping the game winnable for the opponent. A full-power recovery would unfairly neutralise a deserved advantage; a pure 1.5× time boost was marginal with `UCI_LimitStrength` active.
